// components/SubmitInvoices.tsx - Dedicated component for invoice submission
    ...
    try {
      ...
      for (let i = 0; i < validInvoices.length; i++) {
        ...
        try {
          ...
          // Update progress
          const progress = ((i + 1) / validInvoices.length) * 50; // First 50% for copying
      ...
      if (successCount > 0) {
        ...
        for (let i = 0; i < validInvoices.length; i++) {
          ...
          // Update progress
          const progress = 50 + ((i + 1) / successCount) * 50; // Second 50% for deletion
          setSubmitProgress(Math.min(progress, 100));
        }
      ...
      if (failCount === 0) {
        setSubmitMessage(`✅ Successfully submitted ${successCount} invoices!`);
        console.log(`🎉 [SUBMIT] Submission completed successfully: ${successCount} invoices submitted`);
      } else {
        setSubmitMessage(`⚠️ Partially successful: ${successCount} submitted, ${failCount} failed`);
        console.log(`⚠️ [SUBMIT] Submission partially completed: ${successCount} successful, ${failCount} failed`);
        if (errors.length > 0) {
          ...
          <button
            ...
            title={
              !submitButtonState.enabled && submitButtonState.missingPdfs > 0
                ...
                : `Submit ${submitButtonState.validInvoices} valid invoice(s) to permanent storage`
            }
          >
            {isSubmitting ? (
              <>
